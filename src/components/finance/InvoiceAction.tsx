...
export default function InvoiceGenerator({ levels }: InvoiceGeneratorProps) {
  ...
  const handleSubmit = async () => {
      ...
      try {
        ...
        // 2. Read the response body regardless of success/failure
        const data = await res.json();
        
        // 3. Check for error status
        if (!res.ok) {
            // Throw the specific message from the server (e.g., "No active students found")
        ...
        console.error(e);
        // 4. Show the ACTUAL error message to the user
