...
import { ROUTES } from '@/lib/constant'; // Imported fixed routes
import {
...
// --- NAV ITEM COMPONENT (Keep exactly as is) ---
type NavItemProps = {
  href: string;
  label: string;
  icon: LucideIcon;
  ...
  return (
    ...
    >
      ...
      <Icon className={cn("w-5 h-5 transition-colors", 
        isActive ? "text-emerald-700 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-600"
      )} />
...
const SectionHeader = ({ label }: { label: string }) => (
  <div className="mt-6 mb-2 px-4">
    <h3 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest font-sans">{label}</h3>
  </div>
);
...
const Sidebar: React.FC<SidebarProps> = ({ onClose }) => {
  ...
  // --- MENU CONFIGURATION (Fixed Links) ---
  const adminLinks = [
    { href: ROUTES.LEVELS, label: 'Levels', icon: Layers },
    { href: ROUTES.CLASSES, label: 'Classes', icon: School },
    { href: ROUTES.COURSES, label: 'Subjects', icon: BookOpen },
    { href: ROUTES.EXAMS, label: 'Exams', icon: FileBadge },
  ...
  const financeLinks = [
    { href: ROUTES.FINANCE, label: 'Invoices', icon: Receipt }
  ];
  ...
  return (
    <aside className="w-64 bg-white dark:bg-[#0f172a] border-r border-slate-200 dark:border-slate-800 h-full flex flex-col transition-colors duration-300">
      ...
      {/* Nav */}
      <nav className="flex-1 overflow-y-auto py-6 px-3 scrollbar-thin scrollbar-thumb-slate-200">
        ...
        {user.role === 'TEACHER' && (
          <>
              ...
              {teacherLinks.map((link) => <NavItem key={link.href} {...link} pathname={pathname} onClose={onClose} />)}
          </>
        )}
        ...
        {(user.role === 'STUDENT' || user.role === 'PARENT') && (
          <>
            ...
            {studentParentLinks.map((link) => <NavItem key={link.href} {...link} pathname={pathname} onClose={onClose} />)}
          </>
        )}

        <div className="my-4 h-px bg-slate-100 dark:bg-slate-800 mx-2" />
        
        {user.role === 'ADMIN' && <NavItem href={ROUTES.SETTINGS} label="System Settings" icon={Settings} pathname={pathname} onClose={onClose} />}
      </nav>

      {/* Footer */}
      <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <div className="flex items-center gap-3 mb-3 px-1">
          <div className="w-9 h-9 rounded-full bg-white dark:bg-slate-800 border border-slate-200 flex items-center justify-center text-slate-600 font-serif font-bold">
            {user.email.charAt(0).toUpperCase()}
          </div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate">{user.email.split('@')[0]}</p>
            <p className="text-[10px] font-medium text-slate-500 uppercase tracking-wider">{user.role}</p>
          </div>
        </div>
        <button onClick={logout} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 text-slate-600 rounded-xl hover:bg-red-50 hover:text-red-700 transition-all text-xs font-semibold">
